...
public class Okno6 {
    ...
    private void przygotujZdarzenia() {
        ...
        // nowa skladania - wyrazenia lambda
        guzik.addActionListener(ev->{
            int kolor=ThreadLocalRandom.current().nextInt(0x100000);
            guzik2.setBackground(new Color(kolor));
            int dx=ThreadLocalRandom.current().nextInt(20);
            int dy=ThreadLocalRandom.current().nextInt(30);
